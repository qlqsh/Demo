...
- (void)initBackground
{
	...
	[self runAction:[SKAction repeatActionForever:[SKAction playSoundFileNamed:@"game_music.mp3"
...
- (void)initScore
{
	_scoreLabel = [SKLabelNode labelNodeWithFontNamed:@"MarketFelt-Thin"];
...
- (void)createBullets
{
	...
	bullet.zPosition = 1;
	bullet.position = CGPointMake(_playerPlane.position.x,
                                  _playerPlane.position.y + (_playerPlane.size.height/2));
	[self addChild:bullet];
	
	SKAction *actionMove = [SKAction moveTo:CGPointMake(_playerPlane.position.x,
                                                        self.size.height)
                                   duration:0.5];
...
- (void)createFoePlane
{
	...
	if (_bigPlaneTime > 700)
	{
		...
		[self addChild:foePlane];
        [foePlane runAction:[SKAction repeatActionForever:[SKSharedAtles bigFoePlaneAction]]];
		[foePlane runAction:[SKAction sequence:@[[SKAction moveToY:0 duration:speed], [SKAction removeFromParent]]]];
